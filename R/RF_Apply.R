...
#' error which resulted in the final selected model with the final selected
#' variables is extracted. The function also extracts results from the process
#' of random forest fitting to the input data such as OOB errors, predictions on
...
#' @import caret
#' @seealso
#' @return a list of eight elements: 1) a list where each element is a character string
#' containing selected variables from the random forest feature selection process from
#' each seed run, 2) a list where each element is out-of-bag errors from random forest feature
#' selection process of type numeric from each seed run, 3) a list where each element is
#' the OOB error of type numeric from first random forest model fitted during feature
#' selection process from each seed run, 4) a list where each element is a matrix containing
#' confusion matrix of predictions made based on OOB data and first random forest model fitted
#' during feature selection process from each seed run, 5) a matrix containing average
#' importance of features from first random forest model fitted during feature selection process,
#' 6) a list where each element is the OOB error of type numeric from fitted random forest model
#' across each seed run, 7) a matrix containing accuracy and 95% confidence interval for
#' predictions of test data using fitted random forest model for each seed run, and
#' 8) a vector of integers containing seeds used in the procedure.
#' @examples \dontrun{
