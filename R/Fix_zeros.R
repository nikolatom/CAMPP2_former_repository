#' @title Fix zeros
#' @description A function for a detection of zero/negative values and for
#'              replacing zero values in the data. Features having sum of zero
#'              counts higher than the size of the smallest sample group will be
#'              removed by default. Zeros are substituted with minimal values
#'              (>0) observed for each feature.
#' @param data a dataframe of gene/abundance counts.
#' @param group a factor specifying group which should be represented by
...
#'        sample group. Default is TRUE.
#' @export
#' @import impute
#' @seealso
#' @return a data frame with fixed zeros. Features having sum of zero counts
#'         higher than the size of the smallest sample group are removed by
#'         default.
#' @examples \dontrun{
#' ...
#' }
