...
runCampp2 <- function (data1, metadata1, data2=NULL, metadata2=NULL, technology, groups, batches=NULL, data.check=TRUE, standardize=FALSE, transform=FALSE, plot.PCA=FALSE, plot.heatmap=FALSE, kmeans=FALSE, num.km.clusters=NULL, signif=NULL, block=NULL, colors=NULL, prefix="Results", correlation=FALSE, lasso=FALSE, WGCNA=FALSE, cutoff.WGCNA=NULL, survival=FALSE, covariates=NULL, stratify=NULL, surv.plot=50, PPint=FALSE, gene.miR.int=FALSE, show.PCA.labels=FALSE){
    ...
    if (kmeans == TRUE){
        ...
        if(!is.null(data2)){
            if (databatch2 == TRUE){
                Kmeans.Out2 <- runKmeans(data2.batch, show.PCA.labels, colors, paste0(prefix,"_2st_dataset"),num.km.clusters)
            } else {
                if (technology[1] == "seq") {
                    Kmeans.Out2 <- runKmeans(data.frame(data2$E), show.PCA.labels, colors, paste0(prefix,"_2st_dataset"),num.km.clusters)
                } else {
                    Kmeans.Out2 <- runKmeans(data2$E, show.PCA.labels, colors, paste0(prefix,"_2st_dataset"),num.km.clusters) ##not sure if this is necessary
                }
            }
