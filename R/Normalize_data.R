...
#' @param my.variant Possible values are: "array" (mircoarray data), "seq" (high throughput sequencing data), "ms" (mass spectrometry data) or "other" (other type).
...
NormalizeData <- function(my.variant, my.data, my.group, my.transform, my.standardize, my.data.original = NULL) {
    if (my.variant == "seq") {
        ...
        if (my.standardize == "mean") {
            ...
            cat(paste0("\n my.variant = array and my.standardize = mean. Data will be mean centered.", NB, "\n"))

        } else if (my.standardize == "median") {
            rowmed <- apply(my.data,1,median)
            my.data <- my.data - rowmed
            NB <- " N.B This pipeline does not handle background correction of single-channel intensity data or within array normalization two-color intensity data. See limma manual section on array normalization for more on this. Data may be fully normalized with limma (R/Rstudio) or another software and the pipeline re-run."
            cat(paste0("\n my.variant = array and my.standardize = median. Data will be median centered.", NB, "\n"))
        } else if (!(my.standardize %in% c("mean", "median")) & my.variant == "array") {
            my.data <- normalizeBetweenArrays(my.data, method="quantile")
            NB <- " N.B This pipeline does not handle background correction of single-channel intensity data or within array normalization two-color intensity data. See limma manual section on array normalization for more on this. Data may be fully normalized with limma (R/Rstudio) or another software and the pipeline re-run."
            cat(paste0("\n my.variant = array. Data will be normalized on the quantiles.",NB, "\n"))
        } else {
            cat("\n- No standardization requested. If argument my.variant is 'array', data will be normalized on quantile (NormalizeBetweenArrays), otherwise no normalization will be performed.\n")
        }
    } else {
        stop("\n- Option my.variant is mandatory and specifies data type (variant). Options are; array, seq, ms or other. See user manual for specifics.\n")
    }
    return(my.data)
}
