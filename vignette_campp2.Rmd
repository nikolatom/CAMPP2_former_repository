...
##create an object with replaced NAs; features having "NA" in more than 70% 
of the samples will be removed; samples having "NA" in more than 80% 
of the genes will be removed
```
replacedNAs<-ReplaceNAs(data=Na_dataset1, pct.NA.row=70, pct.NA.column=80) 
```
\
...
**Parameters:** \
*data* a dataframe of gene/abundance counts. \
*group* a factor specifying samples' group which should be represented by a column from a metadata file. \
*remove.sparse.features* a logical argument (TRUE/FALSE) for removal of features with sum of zero counts larger than the size of the smallest 
sample group, and for replacing the remaining zeros. Default is TRUE. \
**Output from the function:**  \
a data frame with fixed zeros. Features having sum of zero counts higher than the size of the smallest sample group are removed and remaining zeros will be replaced by default. \
**An example run:** \
...
```
zerofix<-FixZeros(data=campp2_brca_1,group=campp2_brca_1_meta$diagnosis, remove.sparse.features=TRUE)
```
\

### NormalizeData 
**Description:** \
This function normalizes and transforms feature counts depending on data types and selected normalization/standardization methods. 
Transformation methods include options log2, logit and log10. If technology is "seq", a Voom transform is applied automatically.
...
*group* a factor specifying samples' group (e.g. could be represented by a column from a metadata file). \
...
**Output from the function:**  \
Elist (seq technology) or array (array, ms, other technologies) of normalized, 
filtered and transformed feature counts data \
**An example run:** \
...
```
normalized_data<-NormalizeData(data=campp2_brca_1,group=campp2_brca_1_meta$diagnosis,standardize="TMM",transform="voom",technology="seq")
```
