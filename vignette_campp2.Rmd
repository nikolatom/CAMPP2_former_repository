...
**plot.PCA**:  Argument specifies ("TRUE" or "FALSE") if a preliminary PCA plot should be made for data overview. Default is FALSE (do not run).

**PCA.labels** a text ("all", "none) specifying the elements to be labelled. Default value is "none".
...
**plot.umap** This argument specifies ("TRUE" or "FALSE") if a preliminary UMAP plot should be made for data overview. Default is FALSE (do not run).
...
plots with contributions of variables (e.g. genes) to PCA1 and PCA2. 
Top 3 PCA components are visualized in 3D plot.
\
**Parameters:** \
*data* a dataframe of expression/abundance counts \
*group* a factor specifying group for each sample (e.g. could be represented by a column from a metadata file) \
...
PCAPlot(campp2_brca_1_batchCorrected, as.factor(campp2_brca_1_meta$subtype), PCA.labels ="none", cols=NULL, prefix="test_PCA_plot")
```
\
...
In order to speed up the analysis, for data sets with >1000 features, multiple 
subsets (number of sets = number of features/1000; rounded up to the higher 
integer) will be generated, maximum is 10 subsets. A number of randomly selected 
features (e.g. genes) in 1 subset is limited to 2000.
...
the details from PCA are provided (might be used e.g. for a customized PCA plots).
...
**Parameters:** \
*data* a data.frame of feature (e.g. gene) counts \
*PCA.labels* a string vector ("all" or "none") specifying which elements
...
*num.km.clusters* a vector of manually defined number(s) of clusters. By default, the values(s) are calculated
automatically (a default value is NULL). \
**Output from the function:** \
1) a data.frame with cluster information assigned to each sample \
2) a list of results from PCA \
3) PCA plot(s) saved into png \
**An example run:** \

##Estimate number of K-means and assign a specific cluster to each sample
```
###Normalized and batch corrected data ("campp2_brca_1_batchCorrected" generated in a previous step) are used as an input.
Kmeans.output<-runKmeans(campp2_brca_1_batchCorrected, PCA.labels = "all", cols=NULL, prefix="test_kmeans", num.km.clusters=NULL)
```
