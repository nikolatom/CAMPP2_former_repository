...
information (generated by PCA function from FactoMineT package) is
returned. This might be used e.g. for a customized PCA plots.
2) 2D PCA plot(s) projecting samples over first 2 principal components saved
into png.
3) A summary describing the best model is printed on the screen during
calculation.
\
**Parameters:** \
*data* a data.frame of feature (e.g. gene) counts \
...
```
###Normalized and batch corrected data ("campp2_brca_1_batchCorrected" generated in a previous step) are used as an input.
Kmeans.output<-runKmeans(campp2_brca_1_batchCorrected, show.PCA.labels = TRUE, cols=NULL, prefix="test_kmeans", num.km.clusters=NULL)
```
