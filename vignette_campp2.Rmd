...
authors:
  names: Nikola Tom, Mona Nourbakhsh, Matteo Tiberti, Elena Papaleo
...
prefix="test_heatmap", viridis.opt = "turbo", data.type = "expression/abundance")
...
**Output from the function:** \
a list of: \
1) a list where each element is a character string containing selected variables 
from the random forest feature selection process from each seed run. \
2) a list where each element is out-of-bag errors from random forest feature
selection process of type numeric from each seed run. \
3) a list where each element is the OOB error of type numeric from first random 
forest model fitted during feature selection process from each seed run. \
4) a list where each element is a matrix containing confusion matrix of 
predictions made based on OOB data and first random forest model fitted
during feature selection process from each seed run. \
5) a matrix containing average importance of features from first random forest 
model fitted during feature selection process. \
6) a list where each element is the OOB error of type numeric from fitted random 
forest model across each seed run. \
7) a matrix containing accuracy and 95% confidence interval for predictions of 
test data using fitted random forest model for each seed run. \
...
\
**Parameters:** \
*seed* an integer containing a random seed number. \
*data* a matrix of (transformed and normalized) feature counts from
"seq", "array", "ms" or "other" technology (with feature IDs as row names
and sample IDs as columns). \
*group* a factor specifying group for each sample (e.g. could be
