...
```n<-10000``` # a number of genes in the dataset.

```Na_dataset1<-apply (campp2_brca_1, 2, function(x) {x[sample( c(1:n), floor(n/10))] <- NA; x} )``` # For each sample, 1000 NA values is introduced randomly. All 10000 genes (n) in the example dataset are considered.

```replacedNAs<-ReplaceNAs(data=Na_dataset1)``` # creates an object with replaced NAs
\
\
\
